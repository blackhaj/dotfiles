...
if (!bunMarkdown?.html) {
	console.error(
		"Error: Bun.markdown.html is not available. Requires Bun v1.3.8 or newer.",
	);
	process.exit(1);
}
...
function printUsage(): void {
	console.log(
		"Usage: send-markdown.ts <markdown> [--title <title>] [--source <source>] [--url <url>]",
	);
	console.log(
		"       send-markdown.ts --stdin [--title <title>] [--source <source>] [--url <url>]",
	);
}
